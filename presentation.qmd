---
title: '<span style="font-size:0.8em;"><a href="https://info-523-su25.github.io/final-project-howell-xia/" target="_blank">Investigating Sociological & Marginalised Themes in Jokes</a></span>'
subtitle: |
  INFO 523 - Summer 2025 - Final Project  
  <audio controls src="audio/0-intro.mp3"></audio>

  <div style="font-size:0.6em; margin-top:2em; color:#111; text-align:center;">
    Content warning: this project explores sociopolitical themes in humor data; many jokes contain problematic, vulgar, or taboo language.
  </div>
author: '(Ahn) Michael Howell & Cat Xia | <a href="https://github.com/INFO-523-SU25/final-project-howell-xia" target="_blank">Repo Link</a>'


title-slide-attributes:
  data-background-size: stretch
  data-background-opacity: "0.7"
  data-slide-number: none
format:
  revealjs:
    theme:  ['data/customtheming.scss']

resources:
  - audio/**
  
editor: visual
jupyter: python3
execute:
  echo: false
---

```{python}
#| label: load-packages
#| include: false

# Load packages here
import ast # For ensuring preprocessed data is loaded as intended data types
import numpy as np
import pandas as pd
import seaborn as sns # For making plots
sns.set_theme(context = "notebook", style = "whitegrid")
import os # For saving plots
os.makedirs("./images", exist_ok = True)
import matplotlib.pyplot as plt
from sklearn.calibration import LabelEncoder
from sklearn.preprocessing import MinMaxScaler
from sklearn.decomposition import PCA
from sklearn.model_selection import train_test_split

```

```{python}
#| label: setup
#| include: false
#| 
# Set up plot theme and figure resolution
sns.set_theme(style="whitegrid")
sns.set_context("notebook", font_scale=1.1)

import matplotlib.pyplot as plt
plt.rcParams['figure.dpi'] = 300
plt.rcParams['savefig.dpi'] = 300
plt.rcParams['figure.figsize'] = (6, 6 * 0.618)
```

```{python}
#| label: load-data and merge
#| include: false
# Load Data
preprocessed_df = pd.read_csv(
    "./data/preprocessed_dataset.csv",
    converters={"themes_found": ast.literal_eval})

## MERGE DATAFRAMES
jokes_df = preprocessed_df

#removes ID and creates a copy of the joke df for classification
jokes_model = jokes_df.copy()
jokes_model.drop(columns = ["id", "themes_found"], inplace = True)

```


## Our Dataset

<audio controls src="audio/1-dataset.mp3"></audio>

[A dataset of English plaintext jokes](https://github.com/taivop/joke-dataset)

- ~200k jokes from Reddit

- Gathered in 2017 by Taivo Pungas

- Had columns: id, title, body, score

- Why we chose it:

  - Very complete data (few missing values)

  - Text data with sociological topic


## Our Questions

<audio controls src="audio/2-questions.mp3"></audio>

1. What are the distributions of social categories and marginalised themes represented in jokes?
2. How does each sociological theme relate to the scores users assign - are certain themes rated more highly?
  - Can we create a model to predict how well a joke does, through sociological theme alone?

## Our Approach

<audio controls src="audio/3-approach.mp3"></audio>

::: {style="font-size: 80%;"}

Identifying sociological themes

  -  8 themes: gender, ethnicity, sexuality, ability, age, weight, appearance, class

  -  Predefined sets of words to match each theme

:::

::: {style="font-size: 80%;"}

Key files

  - [preprocessing_data.ipynb](https://github.com/INFO-523-SU25/final-project-howell-xia/blob/main/preprocess_data.ipynb)

    - For data preparation & feature engineering

  - [final_analysis_writeup.ipynb](https://github.com/INFO-523-SU25/final-project-howell-xia/blob/main/final_analysis_writeup.ipynb)

    - For analysis & presenting findings

:::

## Our Data Prep (NLP)

<audio controls src="audio/4-data-prep.mp3"></audio>

::: {style="font-size: 80%;"}

Preprocessing steps

  - Combining joke title & body (whole_text)

  - Lowercasing

  - Removing non-letter characters

  - Removing stop words

:::

::: {style="font-size: 80%;"}

Engineering features

  - whole_text, whole_text_normalised_tokens

  - distinct_words, overall_length

  - theme_**theme**_count, theme_**theme**_bool, themes_found

:::

## Example Jokes

<audio controls src="audio/5-example-jokes.mp3"></audio>

![](images/ahn/examples/example-jokes.png)

## Plot 1 - Theme prevalence (Bar)

<audio controls src="audio/6-plot-1-prevalence.mp3" style="display:block; margin:0 0 .75rem auto; width:240px;"></audio>

::: columns
::: {.column width="50%"}
![](images/ahn/1-bar_plot_theme_prevalence_default.png)
:::

::: {.column width="50%"}
![](images/ahn/1-bar_plot_theme_prevalence_strict.png)
:::
:::

::: {style="font-size: 90%;"}
- Most common: gender, ethnicity & sexuality

- Least common: weight, ability

- Default: 28% of jokes have no visible social theme

- Strict: 63% of jokes have only 1 or no theme words
:::

## Plot 2 - Scores by theme (Box)

<audio controls src="audio/7-plot-2-scores-by-theme.mp3"></audio>

![](images/ahn/2-box_plot_scores_default.png){width=60%}

- Themeless jokes score much lower (median: 6 upvotes)

- Uncommon themes (appearance, age, class) get highest scores

- Jokes with ethnic-themed words get lower scores, despite being common

  - Social progress or jokes becoming covert/subtle?

![](images/ahn/examples/ethnicity-leroy-paint.png)

## Plot 3 - Scores by # of themes (Box)

<audio controls src="audio/8-plot-3-scores-by-num-themes.mp3"></audio>

![](images/ahn/3-box_plot_score_vs_num_themes_default.png)

- Clear positive trend: more themes = higher score

- There is a falloff point - joke too long, hard to follow

## Plot 4 - Scores by joke length, by theme (Scatter)

<audio controls src="audio/9-plot-4-scores-by-length.mp3"></audio>

![](images/ahn/4-scatter_plot_length_vs_score_default.png){width=60%}

- Longer jokes receive lower scores

- Jokes with no social theme tend to be shorter

- Otherwise no clear trend separating themes here

## Classification

<audio controls src="audio/10b-classification-approach.mp3"></audio>

![](images/cat/Classification confusion matrix.png)


## Results

<audio controls src="audio/11-classification-results.mp3"></audio>

| Model      | Log Reg  | KNN   | Decision Tree | Rand Forest |
|------------|----------|-------|---------------|-------------|
| Accuracy   | 57%      | 55%   | 57%           | 57%         |
| Precision  | 58%      | 54%   | 58%           | 58%         |
| Recall     | 30%      | 26%   | 29%           | 29%         |
| F1 Score   | 40%      | 35%   | 39%           | 39%         |
| ROC AUC    | 57%      | 55%   | 57%           | 57%         |

## Summary & Next Steps

<audio controls src="audio/12-summary-next-steps.mp3"></audio>

::: {style="font-size: 80%;"}
Q1: What are the distributions of themes represented in jokes?

  - Most jokes involve a social theme, especially gender, ethnicity & sexuality

Q2: How does each theme relate to the scores users assign?

  - Reddit users favor jokes with more social themes
  - Social theme themselves are not enough to predict how well a joke does

**Next steps**

  - Use an LLM for nuanced theme recognition
:::